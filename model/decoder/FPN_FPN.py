...
class FPM(nn.Module):
    ...
    def __init__(self, encoder_channels, pyramid_channel, kernelsize=1):
        ...
        self.c1_down_channel = ConvBlock(
            encoder_channels[0], pyramid_channel, kernel_size=kernelsize, activation=True)
        self.c2_down_channel = ConvBlock(
            encoder_channels[1], pyramid_channel, kernel_size=kernelsize, activation=True)
        self.c3_down_channel = ConvBlock(
            encoder_channels[2], pyramid_channel, kernel_size=kernelsize, activation=True)
        self.c4_down_channel = ConvBlock(
            encoder_channels[3], pyramid_channel, kernel_size=kernelsize, activation=True)
        self.c5_down_channel = ConvBlock(
            encoder_channels[4], pyramid_channel, kernel_size=kernelsize, activation=True)
...
class FPNDecoder(nn.Module):
    def __init__(
        ...
        self.out_channels = pyramid_channel if merge_policy == "add" or merge_policy == "new_policy" else pyramid_channel * 5

        encoder_channels = encoder_channels[1:]

        self.gather1 = FPM(encoder_channels, pyramid_channel)
        self.gather2 = FPM(encoder_channels, pyramid_channel)

        self.separate = FPM([pyramid_channel for _ in range(5)], pyramid_channel, kernelsize=3)

        self.seg_blocks = nn.ModuleList([
            SegmentationBlock(pyramid_channel, change_channels, n_upsamples)
            for n_upsamples in [0, 1, 2, 3, 4]
        ])
        self.merge = MergeBlock(merge_policy, pyramid_channel, pyramid_channel)

    def forward(self, features1, features2):
        features1 = self.gather1(features1[1:])
        features2 = self.gather2(features2[1:])
        diff = [torch.abs(features1[i] - features2[i]) for i in range(5)]

        p1, p2, p3, p4, p5 = self.separate(diff)
